...
선수 초상 상태: 실제 선수 사진을 복제하지 않은 자체 제작 RGBA 초상 11개(홈 타순 9명, 양 팀 선발투수 2명)를 추가했다. 동일한 정적 자산 맵을 경기 시작 선발/전체 라인업, 매 타자 소개, 현재 타자·투수 HUD, FINAL MVP가 공유한다. 런타임 manifest는 총 36개 자산이며 모든 초상은 lazy preload 대상이면서 실제 화면 소비 경로를 가진다.
