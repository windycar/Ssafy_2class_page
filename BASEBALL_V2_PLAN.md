...
- [ ] 주자 sprint/slide/score 보간 애니메이션 구현
- [ ] 홈을 밟는 시점에 presentation 점수 갱신
- [ ] 홈런 2~4초 전용 시퀀스와 스킵 구현

### D. UI와 경기 프레젠테이션

- [ ] `BaseballStage`, `BaseballHUD`, `Scoreboard`, `BatterHUD`, `PitcherHUD` 분리
- [ ] `StrikeZone`, `PitchLayer`, `BattedBallLayer`, `RunnerLayer` 분리
- [ ] `PlayResultOverlay`, `ScoringSequence`, `HomeRunSequence` 분리
- [ ] `GameIntro`, `PlayerIntro`, `HalfInningTransition`, `FinalResult` 분리
- [ ] 현재 타자/투수, 다음 타자 2명, 실제 주자 이름·주력 HUD 표시
- [ ] 득점권/만루/풀카운트/클러치/역전/끝내기 상황 배지 구현
- [ ] 펼칠 수 있는 타석별 PLAY BY PLAY 구현
- [ ] FINAL SCORE, 라인스코어 R/H/E, MVP, 선수 기록, 주요 장면 구현

### E. 이미지 에셋과 성능

- [ ] 같은 경기장/조명/유니폼 스타일의 상황 카메라 최소 15개 제작
- [ ] 타자 portrait 9개 이상, 투수 portrait 2개 이상 제작
- [ ] hit/double/triple/homeRun/strikeout/score/safe/out 효과 또는 상황 컷 제작
- [ ] 전체 새 야구 전용 에셋 25~40개를 실제 화면에 연결
- [ ] 모든 생성 이미지를 직접 열어 잘림·배경·가장자리·스타일·공 중복 검사
- [ ] critical/lazy 그룹을 가진 `GameAssetPreloader` 구현
- [ ] requestAnimationFrame 또는 CSS 변수 기반 공/타구/주자 애니메이션으로 전체 View의 60FPS setState 방지

### F. 온라인 공통 엔진과 안정성

- [ ] 솔로와 온라인은 동일 V2 엔진을 사용하고 Input Provider만 분리
- [ ] 온라인에서 타자 좌석은 스윙/TAKE, 투수 좌석은 구종/코스/제구 입력만 전송
...
- [ ] 4볼 볼넷, 3스트라이크 삼진, 2스트라이크 파울, 3아웃 공수교대
- [ ] single/double/triple/homeRun/grand slam, runner identity, lineup rotation
- [ ] ground/liner/fly/popup, 좌/중/우 안타, 내야 안타, 병살, 희생플라이, 실책
- [ ] 득점/타점/타자 기록/투수 기록/체력/투구 수
