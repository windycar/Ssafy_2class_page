...
- [x] 정적 경기장·초상·효과 39개를 해상도·알파 유지 WebP로 최적화하고 전체 자산을 85.46MB에서 24.17MB 이하로 축소
...
- [x] 직구/슬라이더/커브/체인지업에서 공이 항상 하나만 보이는 시각 검증
- [x] 최소 10타석 또는 한 경기 전체 실제 브라우저 플레이
- [x] 투구, 타격, 안타, 주자, 득점, 홈런, 공수교대, FINAL 화면 캡처 확인
- [x] 3이닝 동안 카메라와 플레이가 단조롭게 반복되지 않는지 확인
- [x] 전체 야구 테스트와 프로덕션 빌드 통과

현재 시각 무결성 상태: 깨진 10프레임 atlas는 파일과 런타임 참조를 모두 제거했다. 각 투구 프레임은 clean RGBA 공 이미지 본체 1개와 공 이미지가 아닌 짧은 모션 블러 10개만 렌더링한다. 실제 엔진의 직구/슬라이더/커브/체인지업을 RELEASE/MID/LATE/PLATE 4단계로 고정한 브라우저 4×4 필름스트립에서 16개 셀 모두 공 이미지 1개·잔상 10개·잔상 이미지 0개를 확인했다. 타자·투수·포수는 같은 2~3등신 스타일의 2048×768 RGBA 4프레임 시트로 통일했으며, 각 시트는 모서리 alpha 0, 프레임 경계 점유 0, 크로마키 초록 번짐 0을 픽셀 회귀 테스트로 고정한다. 실제 크기 타격/투구 화면에서도 타자 방향, 마운드 투수, 홈플레이트 뒤 포수, 얇은 스트라이크존 배치를 확인했다.
...
기록 정확성 상태: 실책 출루 주자별 `earnedRunEligible`과 현재 반이닝의 실책 기반 가상 아웃을 함께 추적한다. 실책을 아웃으로 복원했을 때 가상 3아웃 이후에 들어온 득점은 비자책으로 처리하며, 한 플레이에서 정상 출루 주자와 실책 출루 주자가 함께 득점해도 자책/비자책을 분리한다. FINAL은 양 팀 9명 타자의 PA/AB/H/2B/3B/HR/RBI/R/BB/SO와 투수 IP/Pitches/H/R/ER/BB/SO를 실제 경기 상태에서 만들고, 640px 경기 화면 안에서 키보드 포커스와 세로·가로 스크롤로 전체 기록을 확인할 수 있다.
