...
type UpdateRoom = (room: BangRoom) => unknown;

export function useBangRoomPresence(
  room: BangRoom,
  ...
  const sessionIdRef = useRef(
    typeof crypto !== "undefined" && "randomUUID" in crypto
      ? crypto.randomUUID()
      : `bang-session-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  ...
  useEffect(() => {
    ...
    const leaveForNavigation = (destinationPath: string) => {
      const roomPath = `/games/bang/${roomRef.current.id}`;
      if (destinationPath.startsWith(roomPath)) return;
      bangRoomStorage.leaveRoom(roomRef.current, currentUserId);
    };
    ...
    const handleDocumentClick = (event: MouseEvent) => {
      ...
      if (destination.origin === window.location.origin) {
        leaveForNavigation(destination.pathname);
      }
    };
    ...
    const handlePageHide = () => {
      ...
      navigator.sendBeacon("/api/bang-leave", blob);
    };
