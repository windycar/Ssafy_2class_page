...
export const SSAFY_AI_MOCK_EXAM_ROUND_3: StudyQuestion[] = [
  ...
  // --- [머신러닝 기초 및 회귀 분석] (Q1 ~ Q4) ---
  {
    ...
    prompt: "머신러닝 모델의 복잡도(Complexity)와 학습 데이터 크기에 따른 과적합(Overfitting) 및 과소적합(Underfitting) 특성으로 가장 올바른 것은?",
    options: [
      "모델 복잡도가 지나치게 높으면 훈련 오차는 매우 낮아지지만 일반화 성능이 떨어지며, 데이터양을 늘리면 과적합 완화에 도움이 됨",
      "모델 복잡도가 낮을수록 높은 분산(High Variance)을 보이며 새로운 테스트 데이터에 과민하게 반응함",
  ...
  },
  {
    ...
    category: "머신러닝 기초 및 방법론",
    questionType: "multiple-choice",
    prompt: "다중선형회귀 모델의 성능을 평가할 때 사용하는 결정계수(R^2)와 잔차표준오차(RSE)에 대한 설명으로 가장 올바른 것은?",
    options: [
      "R^2은 각 회귀계수의 p-value를 평균한 값이고, RSE는 독립변수 개수를 의미함",
      "R^2과 RSE는 모두 분류 문제에서만 사용하는 정확도 지표임",
      "R^2은 모델이 종속변수의 변동을 얼마나 설명하는지 나타내고, RSE는 예측값과 실제값 사이에 남는 잔차의 전형적인 크기를 나타냄",
  ...
  },
  {
    ...
    category: "머신러닝 기초 및 방법론",
    questionType: "multiple-choice",
    prompt: "머신러닝의 데이터 구성 요소인 특성(Feature) 및 정답(Label)과 문제 유형의 구분에 대한 설명으로 가장 올바른 것은?",
    options: [
      "입력 특성(Feature)은 항상 1차원 스칼라값이어야 하며 2차원 이상의 벡터는 입력될 수 없음",
      "목표 변수인 Label이 연속적인 수치형 데이터인 문제를 분류(Classification)라고 정의함",
      "지도학습은 Label이 없는 상태에서 데이터의 군집 구조를 찾는 군집화 작업을 포함함",
      "입력 데이터의 속성을 나타내는 변수가 Feature이며, 정답 Label의 형태가 범주형이면 분류, 연속형이면 회귀로 구분함"
    ],
    answer: 3,
  ...
  // --- [자연어 처리 및 시퀀스/LLM] (Q5 ~ Q10) ---
  {
    ...
    questionType: "multiple-choice",
    prompt: "전통적인 원-핫 인코딩(One-hot Encoding) 방식으로 텍스트 단어를 표현할 때 발생하는 본질적인 한계점은?",
    options: [
      "단어 벡터 간의 내적이 항상 0이 되어 단어 사이의 의미적/문맥적 유사도를 전혀 표현할 수 없고 차원이 비대해짐",
  ...
  },
  {
    ...
    category: "자연어 처리 및 텍스트 모델",
    questionType: "multiple-choice",
    prompt: "Word2Vec의 Skip-Gram 학습에서 Window Size=2로 설정했을 때 문맥(Context) 단어의 범위를 가장 올바르게 설명한 것은?",
    options: [
  ...
  },
  {
    ...
    category: "자연어 처리 및 텍스트 모델",
    questionType: "multiple-choice",
    prompt: "순환 신경망(RNN)이 가진 순차적 의존성(Sequential Dependency) 한계를 극복하고 트랜스포머가 전체 문장을 한 번에 병렬 처리할 수 있게 된 핵심 설계는?",
    options: [
      "은닉 상태를 타임스텝마다 디스크에 저장한 후 순차적으로 불러오는 방식",
      "모든 활성화 함수를 시그모이드로 고정하고 게이트를 10개로 확장한 구조",
      "순환 루프(Recurrence)를 완전히 제거하고 위치 인코딩과 셀프 어텐션(Self-Attention)으로 토큰 간 관계를 병렬 연산하는 구조",
  ...
  },
  {
    ...
    category: "자연어 처리 및 텍스트 모델",
    questionType: "multiple-choice",
    prompt: "언어 모델의 대표적인 정량적 평가 지표인 Perplexity(PPL)에 대한 설명으로 가장 올바른 것은?",
    options: [
      "PPL 수치는 높을수록 모델이 다음 단어를 불확실성 없이 정확하게 예측함을 뜻함",
      "PPL은 생성된 텍스트와 정답 텍스트 사이의 n-gram 정밀도(Precision)만을 단독 측정함",
      "PPL은 0과 1 사이의 고정된 확률값으로만 표현되며 음수 값을 가질 수 있음",
      "PPL은 모델이 다음 단어를 예측할 때 느끼는 혼잡도/불확실성을 나타내며, 수치가 낮을수록 언어 모델의 예측 성능이 우수함을 뜻함"
    ],
    answer: 3,
  ...
  },
  {
    ...
    category: "자연어 처리 및 텍스트 모델",
    questionType: "multiple-choice",
    prompt: "LLM의 인컨텍스트 러닝(In-Context Learning)에서 Zero-shot과 Few-shot 프롬프팅의 차이에 대한 설명으로 가장 올바른 것은?",
    options: [
      "Zero-shot은 지시문만 제공하고 Few-shot은 몇 개의 입력-출력 예시를 함께 제공하며, 두 방식 모두 모델의 가중치 업데이트가 일어나지 않음",
  ...
  // --- [CNN 및 대표 비전 아키텍처] (Q11 ~ Q16) ---
  {
    ...
    questionType: "multiple-choice",
    prompt: "CNN에서 합성곱 계층 사이에 적용되는 풀링(Pooling) 계층의 구조적 역할에 대한 설명으로 가장 올바른 것은?",
    options: [
      "특징 맵의 채널 깊이를 2배로 확장하여 비선형 표현력을 늘려줌",
      "학습해야 할 가중치 파라미터가 가장 많이 집중되는 연산 계층임",
      "학습 가능한 파라미터 없이 공간 해상도를 다운샘플링하여 연산량을 줄이고 객체의 미세한 위치 이동에 대한 저항성(불변성)을 제공함",
  ...
  },
  {
    ...
    category: "합성곱 신경망(CNN) 및 대표 비전 모델",
    questionType: "multiple-choice",
    prompt: "합성곱(Convolution) 연산 시 입력 데이터의 외곽 테두리에 0 등의 값을 채워 넣는 패딩(Padding)의 주된 목적은?",
    options: [
      "입력 이미지의 RGB 색상 채널 수를 1개로 압축하기 위함",
      "합성곱 연산의 파라미터 가중치 개수를 0으로 만들기 위함",
      "모든 피처맵의 원소 값을 음수로 변환하여 과적합을 방지하기 위함",
      "가장자리 화소의 특징 정보 손실을 막고 계층이 깊어짐에 따라 공간 해상도가 급격히 축소되는 것을 제어하기 위함"
    ],
    answer: 3,
  ...
  },
  {
    ...
    options: [
      "5x5 1개 방식은 25 C^2, 3x3 2개 중첩 방식은 18 C^2 으로 3x3 중첩 방식이 파라미터를 약 28% 절감함",
  ...
  },
  {
    ...
    options: [
      ...
      "포화되지 않는 비선형 활성화 함수인 ReLU를 도입하여 학습 속도를 가속하고, 완전 연결 계층에 드롭아웃을 적용해 과적합을 방지함",
      "지름길 연결(Skip Connection)을 최초로 적용하여 100층 이상의 심층망을 학습함",
  ...
  },
  {
    ...
    options: [
      ...
      "입력 x가 변형 없이 그대로 전달되는 항등 매핑(Identity Mapping) 상태가 되어 깊은 망에서도 최소한 얕은 망만큼의 성능을 보장함",
  ...
  },
  {
    ...
    category: "합성곱 신경망(CNN) 및 대표 비전 모델",
    questionType: "multiple-choice",
    prompt: "CNN과 비교했을 때 Vision Transformer(ViT)가 갖는 구조적 특성 및 대규모 사전학습의 필요성에 대한 설명으로 가장 올바른 것은?",
    options: [
      "ViT는 이미지 패치의 위치 정보를 자동으로 파악하므로 위치 임베딩이 불필요함",
      "ViT는 소규모 데이터셋(ImageNet-1k)만으로도 항상 ResNet보다 뛰어난 성능을 보임",
      "ViT는 합성곱 필터를 그대로 사용하여 이미지의 국소 연결성을 완벽히 고정함",
      "ViT는 CNN 고유의 국소성 및 이동 불변성 등 귀납적 편향(Inductive Bias)이 적어 대규모 데이터셋 사전학습 시 강력한 전역 표현력을 학습함"
    ],
    answer: 3,
  ...
  // --- [시각-언어 모델(VLM) 및 멀티모달 정합] (Q17 ~ Q20) ---
  {
    ...
    questionType: "multiple-choice",
    prompt: "CLIP 모델에서 이미지 인코더와 텍스트 인코더가 출력한 특징 벡터들을 내적하기 직전에 L2 정규화(Normalize)를 수행하는 수학적 이유는?",
    options: [
      "벡터의 크기(Norm)를 1로 맞춰 단순 행렬 곱(Dot-product) 연산이 두 벡터 간의 코사인 유사도와 수학적으로 정확히 일치하도록 만들기 위해",
  ...
  },
  {
    ...
    options: [
      ...
      "시그모이드 함수의 포화(Saturation) 특성을 통해 이미 거리가 충분히 먼 오답 음성 데이터에 가해지는 불필요한 과도한 오차 페널티를 차단하므로",
  ...
  },
  {
    ...
    options: [
      ...
      "다양한 센서 데이터와 자연스럽게 짝을 맺을 수 있는 이미지/비디오(시각)를 공통 앵커(Anchor)로 삼아 각 모달리티를 정합함",
  ...
  },
  {
    ...
    category: "시각-언어 모델(VLM) 및 멀티모달 정합",
    questionType: "multiple-choice",
    prompt: "LLaVA와 같은 VLM에서 사전학습된 비전 인코더와 거대 언어 모델(LLM) 사이에 선형 프로젝션 레이어를 배치하는 목적은?",
    options: [
      "입력 이미지의 화소 해상도를 무조건 2배로 업샘플링하기 위해",
      "언어 모델의 사전학습된 모든 가중치 파라미터를 초기화하기 위해",
      "텍스트 질문을 음성 주파수 신호로 변환하여 전달하기 위해",
      "비전 인코더가 추출한 시각 특징 벡터의 차원과 공간을 언어 모델의 텍스트 임베딩 차원에 맞추어 정렬(투영)하기 위해"
    ],
    answer: 3,
  ...
  // --- [VLM 변종 및 비전 파운데이션 모델] (Q21 ~ Q24) ---
  {
    ...
    options: [
      ...
      "밝기(Brightness), 채도(Saturation), 명도(Value)",
      "스트라이드(Stride), 패딩(Padding), 커널(Kernel)",
  ...
  },
  {
    ...
    category: "VLM 및 비전 파운데이션 모델",
    questionType: "multiple-choice",
    prompt: "VLM의 시각적 위치 파악(Visual Grounding) 및 UI 조작 에이전트에 Set-of-Mark (SoM) 프롬프팅을 적용했을 때의 효과로 가장 올바른 것은?",
    options: [
      "화면의 해상도를 32x32로 축소하여 연산량을 0으로 만듦",
      "이미지나 UI 요소에 번호 마크를 시각적으로 부여하여 모델이 각 객체를 명확하게 참조하고 선택할 수 있도록 하여 위치 파악 성능을 향상시킴",
  ...
  },
  {
    ...
    category: "VLM 및 비전 파운데이션 모델",
    questionType: "multiple-choice",
    prompt: "Depth Anything v2가 깊이 맵(Depth Map) 예측 성능을 높이기 위해 활용한 데이터 학습 전략으로 가장 올바른 것은?",
    options: [
      "소수의 라벨 데이터만 사용하고 라벨이 없는 이미지는 모두 학습에서 제외함",
      "오디오와 텍스트 데이터만을 사용해 이미지 없이 깊이를 예측하도록 학습함",
      "약 150만 개 규모의 데이터와 함께 약 6200만 개의 라벨되지 않은 데이터를 추가 활용하여 성능을 높임",
  ...
  },
  {
    ...
    options: [
      "객체 탐지(Detection), 광학 흐름(Optical Flow), 이미지 분류, 시맨틱 세그멘테이션",
