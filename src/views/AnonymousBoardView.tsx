...
export default function AnonymousBoardView() {
  ...
  const submit = async () => {
    ...
    const post = { id: createId("post"), title: title.trim(), content: softenAnonymousTone(content), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
    try {
  ...
  return <div className="max-w-3xl mx-auto space-y-5">
    ...
    <section className="space-y-3">{posts.length === 0 ? <p className="text-center text-sm text-gray-400 py-12">아직 게시글이 없습니다.</p> : posts.map((post) => <article key={post.id} className="bg-white border border-border rounded-2xl p-5"><h2 className="font-bold text-gray-800">{post.title}</h2><p className="mt-2 whitespace-pre-wrap text-sm text-gray-600">{post.content}</p><p className="mt-3 text-xs text-gray-400">익명 · {new Date(post.createdAt).toLocaleString("ko-KR")}</p></article>)}</section>
  </div>;
}
